...
class TeamMemberInvitationsInVerticalListViewInfiniteScroll extends StatefulWidget {
  ...
  const TeamMemberInvitationsInVerticalListViewInfiniteScroll({
    super.key,
    this.isAcceptingAll,
    this.isDecliningAll,
  ...
  @override
  State<TeamMemberInvitationsInVerticalListViewInfiniteScroll> createState() => _TeamMemberInvitationsInVerticalListViewInfiniteScrollState();
}

class _TeamMemberInvitationsInVerticalListViewInfiniteScrollState extends State<TeamMemberInvitationsInVerticalListViewInfiniteScroll> {
  ...
  Widget onRenderItem(store, int index, List stores, bool isSelected, List selectedItems, bool hasSelectedItems, int totalSelectedItems) => InvitationItem(
...
class _InvitationItemState extends State<InvitationItem> {
  ...
  /// Request to accept invitation to join team
  Future<bool> requestAcceptInvitationToJoinTeam() {
    
    _startLoader();

    return storeProvider.setStore(store).storeRepository.acceptInvitationToJoinTeam()
    .then((response) {
  ...
  /// Request to decline invitation to join team
  Future<bool> requestDeclineInvitationToJoinTeam() async {
    
    _startLoader();

    return storeProvider.setStore(store).storeRepository.declineInvitationToJoinTeam()
    .then((response) {
      ...
      confirmDismiss: (DismissDirection direction) {
        if(isAcceptingAll == true || isDecliningAll == true) {
          ...
          if(direction == DismissDirection.startToEnd) {
            return requestAcceptInvitationToJoinTeam();
          }else {
            return requestDeclineInvitationToJoinTeam();
          }
